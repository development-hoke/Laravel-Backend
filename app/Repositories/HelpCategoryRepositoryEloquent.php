...
use App\Enums\HelpCategory\EndValue as HelpCategoryEndValue;
...
/**
 * Class HelpCategoryRepositoryEloquent.
 *
 ...
 */
class HelpCategoryRepositoryEloquent extends BaseRepositoryEloquent implements HelpCategoryRepository
{
    use HavingSortTrait;
    ...
    private function mergeNewRootIdAndLevelToAttributes(int $id, array $attributes)
    {
        if (!isset($attributes['parent_id'])) {
            return array_merge($attributes, [
                'parent_id' => null,
                'root_id' => $id,
                'level' => HelpCategoryEndValue::LowestLevel,
            ]);
        }
    ...
    {
        if ($model->level < HelpCategoryEndValue::LowestLevel || $model->level > HelpCategoryEndValue::HighestLevel) {
            throw new InvalidNestedSetAttributesException(__('validation.between.numeric', [
                'attribute' => __('validation.attributes.help_category'),
                'min' => HelpCategoryEndValue::LowestLevel,
                'max' => HelpCategoryEndValue::HighestLevel,
            ]));
        }

        foreach ($model->descendants()->get() as $descendant) {
            if ($descendant->level > HelpCategoryEndValue::HighestLevel) {
        ...
        if (count($ancestors) === 0) {
            if ($model->level !== HelpCategoryEndValue::LowestLevel) {
                throw new InvalidNestedSetAttributesException(error_format('error.inconsistent_level_as_root', [
                    'expected_value' => HelpCategoryEndValue::LowestLevel,
                    'actual_value' => $model->level,
                ]));
            }
        ...
        foreach ($ancestors as $index => $ancestor) {
            ...
            if ((int) $ancestor->level === HelpCategoryEndValue::LowestLevel &&
